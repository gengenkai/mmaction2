...
class ConvTemporalGraphical(nn.Module):
    ...
    def __init__(self,
        ...
        self.PA = nn.Parameter(torch.from_numpy(adj_matrix.astype(np.float32)))
        torch.nn.init.constant_(self.PA, 1e-6)
        self.A = Variable(torch.from_numpy(adj_matrix.astype(np.float32)), requires_grad=False)
...
class STGCN2_2(nn.Module):
    ...
    def __init__(self,
        ...
        spatial_kernel_size = 25
        temporal_kernel_size = 9
        kernel_size = (temporal_kernel_size, spatial_kernel_size)
        self.data_bn = nn.BatchNorm1d(in_channels *
                                      25) if data_bn else identity

        kwargs0 = {k: v for k, v in kwargs.items() if k != 'dropout'}
        self.st_gcn_networks = nn.ModuleList((
            STGCNBlock(
                in_channels, 64, kernel_size, 1, A, residual=False, **kwargs0),
            STGCNBlock(64, 64, kernel_size, 1, A, **kwargs),
            STGCNBlock(64, 64, kernel_size, 1, A, **kwargs),
            STGCNBlock(64, 64, kernel_size, 1, A, **kwargs),
            STGCNBlock(64, 128, kernel_size, 2, A, **kwargs),
            STGCNBlock(128, 128, kernel_size, 1, A, **kwargs),
            STGCNBlock(128, 128, kernel_size, 1, A, **kwargs),
            STGCNBlock(128, 256, kernel_size, 2, A, **kwargs),
            STGCNBlock(256, 256, kernel_size, 1, A, **kwargs),
            STGCNBlock(256, 256, kernel_size, 1, A, **kwargs),
        ))
