...
class AdjMatrixGraph:
    def __init__(self, num_node=25, type='ntu'):
        if type == 'ntu':
            inward_ori_index = [(1, 2), (2, 21), (3, 21), (4, 3), (5, 21), (6, 5), (7, 6),
                                (8, 7), (9, 21), (10, 9), (11, 10), (12, 11), (13, 1),
                                (14, 13), (15, 14), (16, 15), (17, 1), (18, 17), (19, 18),
            ...
            inward = [(0, 1), (2, 1), (3, 2), (4, 3), (5, 1), (6, 5), (7, 6), (8, 1),
...
class UnfoldTemporalWindows(nn.Module):
    ...
    def forward(self, x):
        ...
        N, C, T, V = x.shape
        # print('before unfold--',x .shape) # 12 3  50 25
        x = self.unfold(x)
        # print('after --', x.shape) # 12 9(3x3x1) 1250((50-3+1+2*1)*25)
        # print(self.window_size, self.window_dilation, self.window_stride, self.padding) # 3 1 1 1 
        
        # Permute extra channels from window size to the graph dimension; -1 for number of windows
        x = x.view(N, C, self.window_size, -1, V).permute(0,1,3,2,4).contiguous() # 12 3 3 50 25 -> 12 3 50 3 25
        x = x.view(N, C, -1, self.window_size * V)
        # print(f'after unfold x--{x.shape}\n')  # 12 3  50 75
...
class SpatialTemporal_MS_GCN(nn.Module):
    def __init__(self,
        ...
        if disentangled_agg:
            ...
            A_scales = np.concatenate([normalize_adjacency_matrix(g) for g in A_scales]) # 多尺度 A
        else:
            # Self-loops have already been included in A
            A_scales = [normalize_adjacency_matrix(A) for k in range(num_scales)]
            A_scales = [np.linalg.matrix_power(g, k) for k, g in enumerate(A_scales)] # 多尺度 A
    ...
    def build_spatial_temporal_graph(self, A_binary, window_size):
        ...
        A_large = np.tile(A_binary_with_I, (window_size, window_size)).copy() # 沿每个轴重复的次数
        return A_large
...
class MS_G3D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.out_conv(x).squeeze(dim=3) # 3d Conv
        x = self.out_bn(x)
...
class MSG3D(nn.Module):
    def __init__(self,
        ...
        A_binary = AdjMatrixGraph(num_node=num_point, type=data_type).A_binary 
