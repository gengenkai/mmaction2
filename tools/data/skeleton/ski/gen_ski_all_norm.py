...
def pre_normalization(data, zaxis=[8, 1], xaxis=[2, 5]):
    ...
    for i_s, skeleton in enumerate(s): #  N M T V C
        ...
        for i_p, person in enumerate(skeleton): # M T V C
            if person.sum() == 0: # T V C
                continue
            if person[0].sum() == 0:
    ...
    print('parallel the bone between right shoulder(jpt 8) and '
          'left shoulder(jpt 4) of the first person to the x axis') # 5 right shoulder 2 left shoulder in ski
    prog_bar = mmcv.ProgressBar(len(s))
...
output_train_pkl = '/mnt/lustre/liguankai/data/ski/norm124/train_norm.pkl'
output_val_pkl = '/mnt/lustre/liguankai/data/ski/norm124/val_norm.pkl'
