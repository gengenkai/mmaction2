...
def pre_normalization(data, zaxis=[0, 1], xaxis=[8, 4]):
    ...
    for i_s, skeleton in enumerate(tdqm(s)):
        if skeleton.sum() == 0:
    ...
    for i_s, skeleton in enumerate(tqdm(s)):
        ...
        for i_p, person in enumerate(skeleton):
            ...
            for i_f, frame in enumerate(person):
                if frame.sum() == 0:
                    continue 
                for i,j, joint in enumerate(frame):
...
def gendata(data_path, ignored_sample_path, benchmark='xsub', part='train', pre_norm=True):
    ...
    if filename not in ignored_samples:
        ...
        data = read_xyz(data_path, max_body=max_body_kinect, num_joint=num_joint)
        ...
        anno['keypoint'] = fp[0] 
        anno['keypoint_score'] = np.ones((3, max_frame, num_joint), dtype=np.float32)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Generate Pose Annotation for a single NTURGB-D raw skeleton data')
    parser.add_argument('data_path', type=str, help='')
    parser.add_argument('ignore_sample_path', str=str, default='samples_with_missing_skeletons.txt')
    parser.add_argument('output_folder', type=str, help='output path', default='data/ntu/')
    parser.add_argument('benchmark', type=str, default='xsub')
    parser.add_argument('part', type=str, default='train')
    # parser.add_argument('--device', type=str, default='cuda:0')
    args = parser.parse_args()

    output_path = os.path.join(arg.output_folder, args.benchmark)
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    anno = gendata(args.data_path, args.ignore_sample_path, args.benchmark, args.part)
